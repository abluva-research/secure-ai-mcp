...
func toolsCallHandler(ctx context.Context, id jsonrpc.RequestId, resourceMgr *resources.ResourceManager, body []byte, header http.Header) (any, error) {
	...
	if dbCredsRaw, ok := data["db_credentials"]; ok {
		delete(data, "db_credentials") // Remove so it doesn't get parsed as a tool parameter
		if creds, dynErr := mcputil.ParseDynamicCredentials(dbCredsRaw); dynErr == nil {
			...
			if targetSource != "" && resourceMgr.DynCache != nil {
				...
				if dynErr == nil {
					...
					logger.DebugContext(ctx, fmt.Sprintf("MCP: failed to create dynamic source: %v", dynErr))
